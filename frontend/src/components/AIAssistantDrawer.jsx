...
const DEFAULT_CHIPS = {
  patient: ["Γ₧ò Add Paracetamol 500mg morning", "≡ƒôï Today's schedule", "≡ƒôè My adherence"],
  caregiver: ["≡ƒæÑ List assigned patients", "ΓÜá∩╕Å Check missed dose alerts"],
  admin: ["≡ƒôè System status report", "Γ₧ò Add Ibuprofen to database", "≡ƒÅ╖∩╕Å Low stock alert"]
};
...
const AIAssistantDrawer = () => {
  ...
  const [messages, setMessages] = useState([
    {
      sender: "assistant",
      text: `Hello ${user?.full_name?.split(" ")[0] || ""}! I'm your PillSync AI Assistant ≡ƒÆè.\nHow can I help you today?`,
  ...
  const handleSend = async (textToSend) => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...
        {
          sender: "assistant",
          text: `ΓÜá∩╕Å Error: ${err.message || "Could not process request."}`,
          time: new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
        }
      ]);
    } finally {
