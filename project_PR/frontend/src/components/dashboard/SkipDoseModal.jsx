...
import { AlertTriangle, Pill, Clock, X, Loader2, AlertCircle } from 'lucide-react'
...
export const SkipDoseModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto" role="dialog" aria-modal="true" aria-labelledby="skip-dose-title">
      {/* Backdrop */}
      <div
        ...
        >
          ...
          <div className="flex items-start justify-between pb-4 border-b border-slate-100">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <h3 id="skip-dose-title" className="text-base font-bold text-slate-900 tracking-tight">
                  Skip Medication?
                </h3>
                <p className="text-xs text-slate-500 mt-0.5">Are you sure you want to mark this dose as skipped?</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="mt-4 space-y-4">
            <div>
              <label htmlFor="skip-reason-input" className="block text-xs font-bold text-slate-700 mb-1.5 flex items-center justify-between">
              ...
              <div className="relative">
                <textarea
                  ...
                  placeholder="e.g. Felt nauseous, doctor advised to hold dose"
                  className="w-full px-3.5 py-2 bg-white border border-slate-200 rounded-xl text-xs text-slate-900 placeholder-slate-400 focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 disabled:opacity-60 resize-none"
                />
              </div>
            </div>
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-end space-x-3">
              ...
              <button
                type="submit"
                disabled={isLoading}
                className="inline-flex items-center space-x-1.5 px-4 py-2 rounded-xl text-xs font-semibold text-white bg-amber-600 hover:bg-amber-700 shadow-sm shadow-amber-600/20 transition-all focus:outline-none focus:ring-4 focus:ring-amber-500/20 disabled:opacity-60 cursor-pointer disabled:cursor-not-allowed"
              >
                {isLoading ? (
                  <>
                    <Loader2 className="w-3.5 h-3.5 animate-spin" />
                    <span>Skipping Dose...</span>
                  </>
                ) : (
                  <>
