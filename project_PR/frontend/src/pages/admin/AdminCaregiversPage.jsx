...
export const AdminCaregiversPage = () => {
  ...
  const fetchCaregivers = useCallback(
    async (isSilent = false) => {
      ...
      try {
        ...
        setCaregivers(Array.isArray(data.items) ? data.items : [])
        ...
        setError(
          typeof detail === 'string'
            ? detail
            : 'Unable to load caregiver directory. Please try again.'
        )
      } finally {
        setIsLoading(false)
    ...
    <div className="space-y-6">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-slate-500 mt-1.5 max-w-xl font-medium">
              Directory of authorized caregivers, healthcare organizations, and active patient caseloads.
            </p>
          </div>

          <div className="flex items-center gap-3">
            <button
              onClick={() => fetchCaregivers(true)}
      ...
      <div className="bg-white border border-slate-200/80 rounded-2xl p-4 sm:p-5 shadow-xs flex flex-col md:flex-row md:items-center justify-between gap-4">
        <form onSubmit={handleSearchSubmit} className="flex-1 max-w-md flex items-center gap-2">
          <div className="relative flex-1">
            ...
            <input
              type="text"
              placeholder="Search by caregiver name, email, or organization..."
              value={searchTerm}
              onChange={(e) => setSearchTerm(e.target.value)}
              className="w-full pl-9.5 pr-4 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs text-slate-900 placeholder:text-slate-400 focus:outline-hidden focus:ring-2 focus:ring-sky-500/20 focus:border-sky-500 transition-all"
            />
          </div>
          <button
            type="submit"
            className="px-3.5 py-2 bg-sky-600 hover:bg-sky-700 text-white rounded-xl text-xs font-semibold transition-colors cursor-pointer shadow-xs"
          >
            Search
          </button>
          {activeSearch && (
        ...
        <div className="text-xs text-slate-500 font-medium">
          Showing <strong className="text-slate-800 font-bold">{caregivers.length}</strong> of{' '}
          <strong className="text-slate-800 font-bold">{totalCount}</strong> caregivers
        </div>
      </div>
      ...
      {isLoading ? (
        ...
        <div className="bg-white border border-slate-200 rounded-2xl p-8 text-center shadow-xs">
          ...
          <h3 className="text-base font-bold text-slate-900">Unable to load caregivers</h3>
          <p className="text-xs text-slate-500 max-w-md mx-auto mt-1 font-medium">{error}</p>
          <button
            onClick={() => fetchCaregivers(false)}
        ...
        <div className="bg-white border border-slate-200/80 rounded-2xl p-12 text-center shadow-xs">
          ...
          <h3 className="text-base font-bold text-slate-900">No caregivers found</h3>
          <p className="text-xs text-slate-500 max-w-sm mx-auto mt-1 font-medium">
            {activeSearch
              ? 'No caregiver records matched your search query.'
              : 'No caregiver profiles have been initialized in the system yet.'}
          </p>
        </div>
      ) : (
            ...
            <table className="w-full text-left text-xs">
              <thead className="bg-slate-50/80 border-b border-slate-200/80 text-slate-500 font-semibold uppercase tracking-wider text-[10px]">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Linked Patients</th>
                ...
                {caregivers.map((cg) => (
                  <tr key={cg.id} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-5 py-4">
                      ...
                      <div className="text-[11px] text-slate-400 font-mono mt-0.5">ID: {cg.id.substring(0, 8)}...</div>
                    </td>
                    <td className="px-5 py-4">
                      <div className="font-mono text-slate-700">{cg.email}</div>
                      {cg.phone_number && (
                        <div className="text-[11px] text-slate-400 mt-0.5">{cg.phone_number}</div>
                      )}
                    </td>
                    <td className="px-5 py-4">
                      {cg.organization ? (
                    ...
                    <td className="px-5 py-4">
                      <span className="inline-flex items-center space-x-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-bold bg-teal-50 text-teal-700 border border-teal-200">
                        ...
                        <span>{cg.linked_patients_count} Patients</span>
                      </span>
                    </td>
                    <td className="px-5 py-4">
                      {cg.is_active ? (
                    ...
                    <td className="px-5 py-4 text-right">
                      <Link
                        to={`/app/admin/caregivers/${cg.id}`}
                        className="inline-flex items-center space-x-1 px-3 py-1.5 bg-slate-100 hover:bg-sky-50 text-slate-700 hover:text-sky-700 rounded-xl text-xs font-bold border border-slate-200 hover:border-sky-200 transition-colors cursor-pointer shadow-2xs"
                      >
